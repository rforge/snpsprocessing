...
snpArrayToMatrix <- function(Y, dropNames=TRUE, ...) {
  # Argument 'Y':
  if (!is.array(Y)) {
    stop("Argument 'Y' is not an array: ", class(Y)[1]);
  }
  dim <- dim(Y);
  if (length(dim) != 3) {
  ...
  if (dim[2] != 2) {
    stop("Second dimension of argument 'Y' is not of length 2: ", dim[2]);
  }
  ...
  if (dropNames) {
    ...
    dimnames <- list(NULL, dimnames(Y)[[3]]);
    dim(Y) <- dim;
    dimnames(Y) <- dimnames;
  }
...
snpMatrixToArray <- function(Y, dropNames=TRUE, ...) {
  ...
  # Unstack to an array
  dim <- c(dim[1]/2, 2, dim[2]);
  if (dropNames) {
    dim(Y) <- dim;
  } else {
    dimnames <- list(NULL, c("A", "B"), dimnames(Y)[[2]]);
    dim(Y) <- dim;
    dimnames(Y) <- dimnames;
  }
